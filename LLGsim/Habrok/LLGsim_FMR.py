...
class LLGsim:
    ...
    def animate_m_vector(self):
        ...
        ax3d = fig.add_subplot(121, projection='3d')   # 3D vector subplot
        ax2d = fig.add_subplot(122)                   # components subplot

        # Setup 3D subplot
        ax3d.set_xlim([-1,1]); ax3d.set_ylim([-1,1]); ax3d.set_zlim([-1,1])
        ax3d.set_xlabel('m_x'); ax3d.set_ylabel('m_y'); ax3d.set_zlabel('m_z')
        ax3d.set_title('LLG Magnetization Precession')
        u, v = np.mgrid[0:2*np.pi:30j, 0:np.pi:15j]
        ax3d.plot_wireframe(np.cos(u)*np.sin(v), np.sin(u)*np.sin(v), np.cos(v),
                        color='lightgray', alpha=0.3)
        ax3d.quiver(0, 0, 0, *self.Hext, color='red', length=0.8, normalize=True)
        ax3d.grid(True)
    
        # Setup 2D subplot
        ax2d.set_xlim([0, self.t[-1]])
        ax2d.set_ylim([-1,1])
        ax2d.set_title('Magnetization Components')
        ax2d.set_xlabel('Time [s]')
        ax2d.set_ylabel('m')
        line_mx, = ax2d.plot([], [], label='mx', color='r')
        line_my, = ax2d.plot([], [], label='my', color='g')
        line_mz, = ax2d.plot([], [], label='mz', color='b')
        ax2d.legend()

        # 3D vector and trace
        vector, = ax3d.plot([], [], [], 'o-', color='blue', lw=3)
        ...
        def init():
            ...
            line_my.set_data([], [])
            line_mz.set_data([], [])
            return vector, trace, line_mx, line_my, line_mz
        ...
        def update(frame):
            ...
            # Update 2D component lines
            t_plot = self.t[:frame]
            line_mx.set_data(t_plot, self.m_steps[:frame,0])
            line_my.set_data(t_plot, self.m_steps[:frame,1])
            line_mz.set_data(t_plot, self.m_steps[:frame,2])
            return vector, trace, line_mx, line_my, line_mz

        ani = FuncAnimation(fig, update,
                        frames=range(0, len(self.m_steps), 10),
                        init_func=init,
                        interval=20,
                        blit=True)
    
        ani.save("llg_precession_fmr.gif", writer='pillow', fps=10)
        plt.close(fig)
...
if __name__ == "__main__":
	LLG = LLGsim(Hext=np.array([0, 0, 1]), m0 = np.array([0, 0, 1]), H_drive_amp=1, sigma=None)
